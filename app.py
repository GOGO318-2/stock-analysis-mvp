...
def get_grok_remark(ticker):
    try:
        ...
        data = {
            "model": "grok-beta",
            "messages": [{"role": "user", "content": f"Provide a short remark or additional advice for investing in {ticker}."}]
        }
        response = requests.post(url, headers=headers, json=data)
        content = response.json()['choices'][0]['message']['content']
        return content.strip()
    except:
        return "No additional advice available."
...
# Shared state
currency = info.get('currency', 'USD')  # Auto currency
...
if page == "首页":
    ...
    if not hist.empty:
        ...
        if currency == 'USD':  # US stock pre/post market
    ...
    if info:
        ...
        # Add filter
        trade_type = st.selectbox("选择交易类型", ["所有", "短期交易 (日内/短期)", "趋势交易 (长期)", "波段交易 (中短期)"], index=1)
        if trade_type != "所有":
        ...
        with st.spinner('筛选股票中...'):
            ...
            for tick in tech_stocks:
                try:
                    ...
                    stock_data.append({
                        '股票代码': tick,
                        '公司名称': info.get('longName', tick),
                        '市值': f"{info.get('marketCap', 0):,.0f}",
                        '当天成交额': f"{turnover_avg:,.0f}",
                        '当天成交量': f"{volume_avg:,.0f}",
                        '当前价格': info.get('currentPrice', 0),
                        '今日最高': info.get('dayHigh', 0),
                        '今日最低': info.get('dayLow', 0),
                        '对比昨日涨幅': f"{pct_change:.2f}%",
                        '建议买入等级': buy_level,
                        '买入价格': f"{buy_price:.2f}",
                        '备注': remark
                    })
            ...
            df['等级排序'] = df['建议买入等级'].map({'高': 3, '中': 2, '低': 1})
            df = df.sort_values('等级排序', ascending=False).drop('等级排序', axis=1).head(50)
